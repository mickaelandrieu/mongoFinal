...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var categories = this.db.collection("item").aggregate([
            ...
            var countAll = 0;
            categories.forEach(function countElements(category) {
                countAll += category.num;
            });

            categories.unshift({"_id" : "All", "num" : countAll});
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
        if (category !== "All") {
            var pageItemsQuery = this.db.collection("item").find({"category": category})
                .sort({'_id': 1})
                .limit(itemsPerPage)
                .skip(page * itemsPerPage);
        }else {
            var pageItemsQuery = this.db.collection("item").find()
                .sort({'_id': 1})
                .limit(itemsPerPage)
                .skip(page * itemsPerPage);
        }

        var pageItems = pageItemsQuery.toArray(function(err, pageItems) {
            assert.equal(null, err);
            callback(pageItems);
        });
    }
